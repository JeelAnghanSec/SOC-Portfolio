...
```mermaid
flowchart TD
    A[Windows 10 Endpoint] --> B[Windows Event Logs]
    A --> C[Sysmon]
    B --> D[Wazuh Agent]
    C --> D
    D --> E[Wazuh Manager]
...
Sysmon provides detailed endpoint telemetry — such as process creation, network connections, and other system activity — that supports security investigations.

---

## 🔍 Security Investigations

Status of each investigation planned for this project:
...
Sysmon is installed and running on the Windows 10 endpoint as a Windows service, providing detailed endpoint telemetry for security monitoring and investigation.
...
The Sysmon Operational log contains detailed endpoint telemetry — including process creation, image loading, and registry changes — that the SOC can analyze to identify unusual or potentially suspicious activity on the Windows endpoint.
...
The PowerShell process creation event was successfully collected from the Windows endpoint by the **Wazuh Agent** and indexed in Wazuh, confirming that Sysmon process telemetry is available for centralized SOC investigation.
...
> **Analysis:** Wazuh successfully received and indexed the Sysmon process telemetry, allowing the SOC analyst to investigate PowerShell execution centrally. This IP address belongs to the lab's Windows endpoint and is documented here as host context, not as a malicious indicator.
...
```mermaid
flowchart TD
    A[Activity / Attack] --> B[Log Generation]
    B --> C[Wazuh Agent Collection]
    C --> D[Wazuh Detection]
    D --> E[Alert Triage]
    E --> F[Investigation]
    F --> G[IOC Analysis]
...
For each completed investigation, the following information is documented:
